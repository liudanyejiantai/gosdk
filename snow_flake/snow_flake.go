// Copyright 2018 twitter Authors
// package snow_flake
// snowflake ID 算法是 twitter 使用的唯一 ID 生成算法，为了满足 Twitter 每秒上万条消息的请求
// 使每条消息有唯一、有一定顺序的 ID ，且支持分布式生成。
// 使用说明 	IdWorker, err := snow_flake.NewWorker(0)
// 上面的0表示传入当前节点id 此id在机器集群中一定要唯一 且从0开始排最多1024个节点
// id := IdWorker.GetId() //获得唯一id,类似68655106454716416
package snow_flake
